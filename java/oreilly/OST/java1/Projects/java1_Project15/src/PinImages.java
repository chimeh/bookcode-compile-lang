...
public class PinImages {
  ...
  public void drawCookie(Graphics g, Color col, int x, int y, int width, int height) {
    // I "heart" Duke
    g.setColor(col);
    g.fillOval(x, y, width, height);
    g.setColor(Color.pink);
    g.fillOval(x+((width*12)/100), y+((height*12)/100), (width*80)/100, (height*80)/100);
   
    
  }
  
  public void fillTriangle(Graphics g, int bottomX, int bottomY, int base, int height) {
    // isosceles base at the bottom
