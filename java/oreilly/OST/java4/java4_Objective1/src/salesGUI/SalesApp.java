...
public class SalesApp {
  ...
  public static void main(String[] args) {
    SalesApp myApp = new SalesApp();
    int[] salesValues = {4,6,7,8};
    
    
    myApp.setSales(salesValues);
    System.out.println("Average sales is " + myApp.getAverage());
    myApp.calculateMinMax();
    myApp.setSalesBar(6);
    System.out.println("Salesbar is " + myApp.getBar());
    
    for (int i = 0; i < myApp.determineTopSalesPeople().length; i ++) {
      if (myApp.determineTopSalesPeople()[i] == 1) {
        System.out.println("Index " + i + "(Salesman " + (i+1) + ") sold " + myApp.getSales()[i]);
      }
    }
  }
